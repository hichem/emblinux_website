...
$data = array("UDP","RPC/UDP","TCP","RPC/TCP","TCP_Con");

for($i=0;$i<$max_rep;$i++)
{if(strcmp($xml[$i]->network_communication_latency->udp_lat_net,"     ")==0)
{$data1[$nom_dir[$i]][0]=0;}
else{$data1[$nom_dir[$i]][0]=(float)($xml[$i]->network_communication_latency->udp_lat_net);}
if (strcmp($xml[$i]->network_communication_latency->udp_rpc_lat_net,"     ") == 0)
{$data1[$nom_dir[$i]][1]=0;}
else{$data1[$nom_dir[$i]][1]=(float)($xml[$i]->network_communication_latency->udp_rpc_lat_net);}
if (strcmp($xml[$i]->network_communication_latency->tcp_lat_net,"     ")==0)
{$data1[$nom_dir[$i]][2]=0;}
else{$data1[$nom_dir[$i]][2]=(float)($xml[$i]->network_communication_latency->tcp_lat_net);}
if (strcmp($xml[$i]->network_communication_latency->tcp_rpc_lat_net,"     ")== 0)
{$data1[$nom_dir[$i]][3]=0;}
else{$data1[$nom_dir[$i]][3]=(float)($xml[$i]->network_communication_latency->tcp_rpc_lat_net);}
if (strcmp($xml[$i]->network_communication_latency->tcp_connec_lat_net,"    ")== 0)
{$data1[$nom_dir[$i]][4]=0;}
else{$data1[$nom_dir[$i]][4]=(float)($xml[$i]->network_communication_latency->tcp_connec_lat_net);}
}
...
// TITRE: texte
$graph->title->Set("Remote Communication latencies in microseconds");
