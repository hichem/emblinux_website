...
$data = array("$ L1","$ L2","Main mem","Rand mem","guesses");

for($i=0;$i<$max_rep;$i++)
{if(strcmp($xml[$i]->memory_latencies->L1,"      ")==0)
{$data1[$nom_dir[$i]][0]=0;}
else{$data1[$nom_dir[$i]][0]=(float)($xml[$i]->memory_latencies->L1);}
if (strcmp($xml[$i]->memory_latencies->L2,"      ")== 0)
{$data1[$nom_dir[$i]][1]=0;}
else{$data1[$nom_dir[$i]][1]=(float)($xml[$i]->memory_latencies->L2);}
if (strcmp($xml[$i]->memory_latencies->mainmem_lat,"           ") == 0)
{$data1[$nom_dir[$i]][2]=0;}
else{$data1[$nom_dir[$i]][2]=(float)($xml[$i]->memory_latencies->mainmem_lat);}
if (strcmp($xml[$i]->memory_latencies->randmem_lat,"           ") == 0)
{$data1[$nom_dir[$i]][3]=0;}
else{$data1[$nom_dir[$i]][3]=(float)($xml[$i]->memory_latencies->randmem_lat);}
if (strcmp($xml[$i]->memory_latencies->guesses_lat," ") == 0)
{$data1[$nom_dir[$i]][4]=0;}
else{$data1[$nom_dir[$i]][4]=(float)($xml[$i]->memory_latencies->guesses_lat);}
}
...
// TITRE: texte
$graph->title->Set("Memory latencies in nanoseconds ");
