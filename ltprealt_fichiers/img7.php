...
for($i=0;$i<$max_rep;$i++)
{ 
$data1[$i]=(float)($xml[$i]->Pthread_Kill_Latency->min);
}
for($i=0;$i<$max_rep;$i++)
{ 
$data2[$i]=(float)($xml[$i]->Pthread_Kill_Latency->max);
}
for($i=0;$i<$max_rep;$i++)
{ 
$data3[$i]=(float)($xml[$i]->Pthread_Kill_Latency->avg);
}
for($i=0;$i<$max_rep;$i++)
{ 
$data4[$i]=(float)($xml[$i]->Pthread_Kill_Latency->stddev);
}
...
// TITRE: texte
$graph->title->Set("Pthread kill latency -Mesuring Min(us)-");
...
// TITRE: texte
$graph1->title->Set("Pthread kill latency -Mesuring Max(us)");
// TITRE: marge et apparence
...
// TITRE: texte
$graph2->title->Set("Pthread kill latency -Mesuring Average(us)");
// TITRE: marge et apparence
